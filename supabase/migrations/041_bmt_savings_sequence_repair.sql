...
-- Older local seed rows predate number_sequences. Seed the shared savings
-- sequence from the existing maximum before new customer accounts are opened.
...
BEGIN
    ...
    VALUES (auth.uid(), v_customer.branch_id, 'SAVINGS_ACCOUNT_OPENED', 'FINANCIAL_ACCOUNT', v_account_id, jsonb_build_object('product_id', v_product.id, 'account_number', v_branch_code || '.' || v_sequence), jsonb_build_object('channel', 'CUSTOMER_PORTAL'));
    RETURN v_account_id;
END; $$;
