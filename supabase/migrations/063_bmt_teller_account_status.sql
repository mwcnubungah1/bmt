...
DROP FUNCTION IF EXISTS bmt_db.teller_search_savings_accounts(TEXT);
...
BEGIN
  ...
  RETURN QUERY SELECT fa.id, fa.account_number::TEXT, c.id, c.cif_number::TEXT, c.full_name::TEXT, fa.status::TEXT, sa.current_balance, fa.branch_id
  FROM bmt_db.financial_accounts fa JOIN bmt_db.customers c ON c.id = fa.customer_id JOIN bmt_db.savings_accounts sa ON sa.financial_account_id = fa.id
  WHERE fa.account_type = 'SAVINGS' AND (NULLIF(btrim(p_search), '') IS NULL OR c.full_name ILIKE '%' || btrim(p_search) || '%' OR c.cif_number ILIKE '%' || btrim(p_search) || '%' OR fa.account_number ILIKE '%' || btrim(p_search) || '%')
