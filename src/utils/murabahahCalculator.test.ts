...
describe('calculateMurabahah', () => {
  it('calculates Rp50m, 12 months, 12% flat margin', () => {
    ...
    expect(result.monthlyInstallment).toBe(4_666_666.66)
    expect(result.adminFee).toBe(500_000)
    expect(result.schedule).toHaveLength(12)
    expect(result.schedule.reduce((sum, row) => sum + row.totalAmount, 0)).toBeCloseTo(56_000_000, 2)
  })
  ...
  it('puts rounding remainder in the final installment', () => {
    ...
    expect(result.schedule.map((row) => row.totalAmount)).toEqual([36.66, 36.66, 36.68])
    expect(result.schedule.reduce((sum, row) => sum + row.totalAmount, 0)).toBeCloseTo(result.sellingPrice, 2)
  })
  ...
  it('rejects negative admin fees and preserves the exact selling price total', () => {
    expect(() => calculateMurabahah({ principal: 100, tenorMonths: 3, marginRate: 10, adminFeeType: 'FIXED', adminFeeValue: -1 })).toThrow('administrasi')
    const result = calculateMurabahah({ principal: 100, tenorMonths: 3, marginRate: 10 })
